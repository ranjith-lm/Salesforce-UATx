({
    doInit : function (component, event, helper, accId) {
        var action = component.get('c.getDeceasedCustomerCaseRecordType');
    ...
    validatePhoneNumber : function(component, event, helper) {
        var phoneNumber = component.get("v.SuppliedPhone");
        var phonePattern = /^[+]?[0-9\s-()]*$/; // Simple regex for international numbers
        if (!phoneNumber.match(phonePattern)) {
            helper.handleErrors('Please enter a valid phone number.', '');
            return false;
        }
        return true;
    }
