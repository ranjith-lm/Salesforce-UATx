...
*/
({
    getAccountToBlockUnblock: function (component, event, helper, accId) {
        var action = component.get('c.getAccountToBlockUnblockViaApi');
        action.setParams(
            {
                accountId: accId,
        ...
        action.setCallback(this, function (actionResult) {
            ...
            if (statut === "SUCCESS") {
                ...
                if (data) {
                    ...
                    component.set("v.accToBlockUnblockList", data);
                    var plValues = [];
                    var allacnt=[];
                    var str = '';
                    for (var i = 0; i < data.length; i++) {
                        plValues.push({
                            label: data[i].ac_label,
                            value: data[i].iban
                        });
                        if(i==0){
                            str = data[i].ac_label+',';
                    ...
                    component.set('v.allAccList', str);
                    component.set("v.AccList", plValues);
                }
